...
class FeedForwardLanguageModel(nn.Module):
    ...
    def forward(self, word_indices: list[Tensor]) -> Tensor:
        ...
        embeddings = [self.embedding(i) for i in word_indices]
        concatenated_embeddings = ops.concat(*embeddings)
        fc_output = self.fc(concatenated_embeddings)
        tanned = ops.tanh(fc_output)
        final = self.output(tanned)
        return final