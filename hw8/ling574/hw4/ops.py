...
def softmax_rows(logits: Tensor) -> Tensor:
    ...
    expo = exp(logits)
    sum_exp = sum_along_columns(expo)
    return divide(expo, sum_exp)
...
def cross_entropy_loss(probabilities: Tensor, labels: Tensor) -> Tensor:
    ...
    prob_logs = log(probabilities)
    label_logs = multiply(prob_logs, labels)
    sum_label_logs = sum_along_columns(label_logs)
    mean = reduce_mean(sum_label_logs)
    return multiply(mean, negative_one)
