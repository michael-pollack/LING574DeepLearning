...
class LSTMCell(nn.Module):
    ...
    def forward(
        ...
        forget_gate = torch.sigmoid(self.forget_linear(combined_input))
        input_gate = torch.sigmoid(self.input_linear(combined_input))
        candidate_memory = torch.tanh(self.candidate_linear(combined_input))
        memory = (forget_gate * prev_memory) + (input_gate * candidate_memory)
        output_gate = torch.sigmoid(self.output_linear(combined_input))
